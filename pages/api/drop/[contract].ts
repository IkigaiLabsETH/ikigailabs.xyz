...
const get = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const clientContract = await client
      .getContract(...args)
    const result = {}
    ...
    if (type === 'nft-drop') {
      ...
      result['claimedSupply'] = await claimedSupplyPromise.then((supply) => parseInt(supply.toString()))
      result['unclaimedSupply'] = await unclaimedSupplyPromise.then((supply) => parseInt(supply.toString()))
