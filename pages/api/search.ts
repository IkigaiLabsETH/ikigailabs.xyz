...
export const handler = (req: NextApiRequest, res: NextApiResponse) => 
  match(req.method)
    .with('GET', async () => {
      const { query: { query, chains } } = req
      const chainList = pipe(split(','), map(parseInt))(chains as string)
      const selectedChains = isEmpty(chainList) ? supportedChains : filter((chain: ReservoirChain) => includes(chain.id)(chainList))(supportedChains)

      // Search by address
      if (isAddress(query as string)) {
        const result = await Promise.allSettled<{status: string, value: any}>(map((chain: ReservoirChain) => fetch(`${chain.reservoirBaseUrl}/collections/v7/?contract=${query}&excludeSpam=true`).then(response => response.json()))(selectedChains))
        const collections = reduce((acc, { status, value }) => {
          if (status === 'fulfilled') {
            return {
