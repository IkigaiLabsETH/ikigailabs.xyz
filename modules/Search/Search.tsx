...
import { filterOutChains, toggleListItem, findChainIconByChainId, findChainNameByChainId, formatAmount, formatNumber } from '../../common/utils'
  ...
  return (
    <div className={clsx('relative transition-all ease-in-out duration-300', inFocus ? 'w-full': 'w-1/3')}>
      <div onClick={(e) => {e.stopPropagation()}}>
        <div className='flex flex-row items-center bg-black text-yellow p-1 px-2 border-2 border-yellow' onFocus={() => setInFocus(true)}>
          <FaMagnifyingGlass />
          <input
            type="text"
            value={prefix}
        ...
        {inFocus ? 
          <div className="absolute w-full">
            <div className={clsx('grid grid-cols-13 w-full p-1 px-2 mt-1 overflow-x-auto overflow-y-hidden bg-neutral-950')}>
              {map((chain: ReservoirChain) => (
                <div className={clsx('hover:cursor-pointer hover:bg-black flex justify-center items-center p-2 py-4 border-b-2 hover:border-yellow ', includes(chain.id)(selectedChains) ? 'border-yellow bg-black' : 'border-b-neutral-950')} onClick={() => updateChainSelection(chain.id)}>
        ...
        <div className='absolute w-full mt-[4.5rem]'>  
          <div>
            {status === QueryStatus.pending && (
              <>
                <div><SkeletonLoader height='h-8' /></div>
                <div><SkeletonLoader height='h-8' /></div>
                <div><SkeletonLoader height='h-8' /></div>
              </>
            )}
                            ...
                            <div className='flex flex-col items-start'>
                              <div className='font-bold text-lg mr-2 flex flex-row'>
                                ...
                                <div className='w-3 h-3 mt-1 ml-1.5'>
                                  <img src={`/assets${findChainIconByChainId(collection.chainId)}`} alt={findChainNameByChainId(collection.chainId)} />
                                </div>
                              </div>
                              
                              <div className='text-xs font-bold text-slate-500 flex flex-row items-center'>
                                {collection.openseaVerificationStatus === 'verified' ? <img src={`/assets/icons/opensea-verified.svg`} alt="OpenSea verified" /> : null}
