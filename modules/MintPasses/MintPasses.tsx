...
import { joinAllowlist, selectAllowlistLoadingState, selectTokens } from './mintPasses.slice'
      ...
      <div className="flex flex-row space-x-6 mt-32">
        <div className="w-full">
          ...
          <p className="text-black text-xl lg:text-2xl">
            ...
            discerning collectors. Mint Passes act as access cards to our community of digital artists and
            collectors as well as its exclusive events, including art and cultural trips, private auctions, gallery
            openings, and artist dinners. It unlocks priority mints from our roster of iconic and emerging artists!{' '}
          </p>

          {
            map(({ name }: { name: string }) => (
              <div key={name} className="text-[2rem] md:text-[4rem] lg:text-[5rem] leading-none boska font-bold border-b border-b-gray-400 py-5"><button onClick={console.log} className="w-full text-left flex justify-between items-center p-2 hover:text-red transition-colors">{name} <ChevronRightIcon className='w-8'/></button></div>
            ))(mintPasses)
          }
