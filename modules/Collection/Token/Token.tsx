...
export const Token: FC<TokenProps> = ({ contract, tokenId }) => {
  ...
  const component = () => {
    const { token: { image, name, description, attributes, owner, contract, tokenId, kind }, market: { floorAsk, topBid } } = token as any
            ...
            <div className="w-2/3">
              ...
              <div className='border-t-2 mt-8 border-black mr-16'>
                <ul className='text-gray-800 text-xs p-4 pl-0'>
                  <li className='mb-1'><span className='font-bold'>Contract:</span> {contract}</li>
                  <li className='mb-1'><span className='font-bold'>Token Id:</span> {tokenId}</li>
                  <li className='mb-1'><span className='font-bold'>Standard:</span> {kind}</li>
                </ul>
              </div>
            </div>
            <div className="w-1/3">
              <div className='border-2 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] p-4 mb-6'>
                ...
                <p className='text-s text-black pl-1 mb-0'><span className='font-bold'>Owner:</span> {owner}</p>
              </div>
              {attributes && (
                  <ul className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-5">
                    {map((attribute: { key: string; value: string, floorAskPrice: number | null, onSaleCount: number, tokenCount: number }) => (
                      <li
                        key={attribute.value}
